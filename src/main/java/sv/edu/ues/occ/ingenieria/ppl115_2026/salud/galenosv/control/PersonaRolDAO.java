...
public class PersonaRolDAO extends DefaultDAO<PersonaRol>{
    ...
    public EntityManager getEntityManager() {
     ...
     @Override
    public List<PersonaRol> findRange(int first, int max) throws IllegalArgumentException, IllegalStateException {
        if (first >= 0 && max > 0) {
            try {
                TypedQuery<PersonaRol> q = em.createNamedQuery("PersonaRol.findAll", PersonaRol.class);
