...
public class ExamenDAO extends DefaultDAO<Examen> {
    ...
    public EntityManager getEntityManager() {
     ...
     @Override
    public List<Examen> findRange(int first, int max) throws IllegalArgumentException, IllegalStateException {
        if (first >= 0 && max > 0) {
            try {
                TypedQuery<Examen> q = em.createNamedQuery("Examen.findAll", Examen.class);
