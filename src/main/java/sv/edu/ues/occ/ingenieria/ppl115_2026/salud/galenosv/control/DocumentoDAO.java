...
public class DocumentoDAO extends DefaultDAO<Documento> {
    ...
    public EntityManager getEntityManager() {
     ...
     @Override
    public List<Documento> findRange(int first, int max) throws IllegalArgumentException, IllegalStateException {
        if (first >= 0 && max > 0) {
            try {
                TypedQuery<Documento> q = em.createNamedQuery("Documento.findAll", Documento.class);
