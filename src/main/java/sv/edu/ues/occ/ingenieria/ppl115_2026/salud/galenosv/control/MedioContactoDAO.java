...
public class MedioContactoDAO extends DefaultDAO<MedioContacto>{
    ...
    public EntityManager getEntityManager() {
     ...
     @Override
    public List<MedioContacto> findRange(int first, int max) throws IllegalArgumentException, IllegalStateException {
        if (first >= 0 && max > 0) {
            try {
                TypedQuery<MedioContacto> q = em.createNamedQuery("MedioContacto.findAll", MedioContacto.class);
