...
public class ConsultaProcedimientoPasoDAO extends DefaultDAO<ConsultaProcedimientoPaso> {
  ...
  @Override
    public List<ConsultaProcedimientoPaso> findRange(int first, int max) throws IllegalArgumentException, IllegalStateException {
        if (first >= 0 && max > 0) {
            try {
                TypedQuery<ConsultaProcedimientoPaso> q = em.createNamedQuery("ConsultaProcedimientoPaso.findAll", ConsultaProcedimientoPaso.class);
