...
import sv.edu.ues.occ.ingenieria.ppl115_2026.salud.galenosv.entity.TipoDocumento;
...
public class TipoDocumentoDAOTest {
    ...
    List<TipoDocumento> LISTA_REGISTROS;

    
    public TipoDocumentoDAOTest() {
           LISTA_REGISTROS = new ArrayList<>();
        LISTA_REGISTROS.add(new TipoDocumento(UUID.randomUUID()));
  ...
  @Test
    public void testFindRange() {
        ...
        Mockito.when(mockEM.createNamedQuery("TipoDocumento.findAll", TipoDocumento.class)).thenReturn(mockTQ);
        TipoDocumentoDAO cut = new TipoDocumentoDAO();
        cut.em = mockEM;
        List<TipoDocumento> resultado = cut.findRange(first, max);
