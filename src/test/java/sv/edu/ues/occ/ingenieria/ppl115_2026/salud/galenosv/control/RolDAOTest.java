...
import sv.edu.ues.occ.ingenieria.ppl115_2026.salud.galenosv.entity.Rol;
...
public class RolDAOTest {
   ...
   List<Rol> LISTA_REGISTROS;

    
    public RolDAOTest() {
           LISTA_REGISTROS = new ArrayList<>();
        LISTA_REGISTROS.add(new Rol(UUID.randomUUID()));
  ...
  @Test
    public void testFindRange() {
        ...
        Mockito.when(mockEM.createNamedQuery("Rol.findAll", Rol.class)).thenReturn(mockTQ);
        RolDAO cut = new RolDAO();
        cut.em = mockEM;
        List<Rol> resultado = cut.findRange(first, max);
